...
function SEOMeta({ title, description, keywords, img }: SEOMetaProps) {
  let setTitle =
    title ??
    'Reflio: Create a privacy-friendly referral program for your SaaS.';
  let setDescription =
    description ??
    'Create a privacy-friendly referral program for your SaaS. GDPR Friendly. Based in the UK. European-owned infrastructure.';
  let setKeywords =
    keywords ??
    'Reflio, Referral software, create referral program, stripe referral program';
  ...
  return (
    <Head>
      ...
      <meta
        key="twImage"
        name="twitter:image"
        content={`${process.env.NEXT_PUBLIC_AFFILIATE_SITE_URL}${setImg}`}
      />
      ...
      <meta
        key="ogURL"
        property="og:url"
        content={`${process.env.NEXT_PUBLIC_AFFILIATE_SITE_URL}`}
      />
      <meta
        key="ogImage"
        property="og:image"
